   ...
   while (1) {
      ...
      if (fds[0].revents & POLLIN) {
         ...
         if (userNumber < MAX_USER) {
            ...
            // Send message
            char *message =
                "Please sign in to continue (client_id: client_name)\n";
      ...
      for (int i = 1; i < nfds; ++i) {
         if (fds[i].revents & (POLLIN | POLLERR)) {
            ...
            if (ret <= 0) {
               ...
               int j = 0;
               for (; j < userNumber; ++j) {
                  if (users_socket[j] == client) break;
               }
            ...
            if (j == userNumber) {
               ...
               if (ret == 2) {
                  ...
                  if (strcmp("client_id:", cmd) == 0) {
                     ...
                     userNumber++;
                     char *message =
                         "Login successfully (format to chat private: "
                         "PRIVATE_TO_clientid message)\n";
                     send(client, message, strlen(message), 0);
                     continue;
                  }
               }
               ...
               if (strncmp("PRIVATE_TO_", buff, 11) == 0) {
                  ...
                  if (tmp == NULL) {
                     // Wrong format
                     char *message =
                         "Wrong format to private chat (PRIVATE_TO_clientid "
                         "message)\n";
                     send(client, message, strlen(message), 0);
                     continue;
                  }
                  ...
                  for (des; des < userNumber; ++des) {
                     if (strcmp(users_id[des], id) == 0) break;
                  }
                  ...
                  // boardcast message
                  char message[256];
                  sprintf(message, "%04d/%02d/%02d %02d:%02d:%02d ",
