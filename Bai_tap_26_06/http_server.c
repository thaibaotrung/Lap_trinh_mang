...
void signal_handler(int signo) { wait(NULL); }
...
void *client_thread(void *param) {
   ...
   if (strncmp(buff, "GET /get", 8) == 0) {
      ...
      if (strncmp(query, "file=", 5) == 0) {
         ...
         send(clientSocket, message, strlen(message), 0);
         sprintf(buff,
                 "<h1><a style=\"font-weight:900;color:#00008B\" "
                 "href=\"http://localhost:%d/get?dir=%s\">.</a></h1>",
                 port, directory);
         ...
         while ((entry = readdir(dir)) != NULL) {
            if (strcmp(entry->d_name, ".") != 0 &&
               ...
               if (entry->d_type == DT_REG) {
                  sprintf(buff,
                          "<h1><i><a style=\"font-weight:100;color:#00d2ff\" "
                          "href=\"http://localhost:%d/get?file=%s/%s\">%s</a></"
                          "i></h1>",
                          port, directory, entry->d_name, entry->d_name);
                  send(clientSocket, buff, strlen(buff), 0);
               } else {
                  sprintf(
                      buff,
                      "<h1><a style=\"font-weight:900;color:#00008B\" "
                      "href=\"http://localhost:%d/get?dir=%s/%s\">%s</a></h1>",
                      port, directory, entry->d_name, entry->d_name);
                  send(clientSocket, buff, strlen(buff), 0);
               }
            }
         ...
         if (strcmp(entry->d_name, ".") != 0 &&
            ...
            if (entry->d_type == DT_REG) {
               sprintf(buff,
                       ...
                       "href=\"http://localhost:%d/get?file=%s/%s\">%s</a></"
                       "i></h1>",
                       port, cwd, entry->d_name, entry->d_name);
               send(clientSocket, buff, strlen(buff), 0);
            } else {
               sprintf(buff,
                       "<h1><a style=\"font-weight:900;color:#00008B\" "
...
int main(int argc, char *argv[]) {
   ...
   while (1) {
      int client = accept(serverSocket, (struct sockaddr *)&clientAddr,
                          &clientAddrLength);
      if (client == -1) {
