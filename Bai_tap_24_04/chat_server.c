...
int main(int argc, char *argv[]) {
   ...
   while (1) {
      ...
      if (FD_ISSET(serverSocket, &fdread)) {
         ...
         // Ask for client name
         send(clientSocket, "Please enter your name (client_id:client_name)",
              46, 0);
      }
      ...
      for (int i = 0; i < numberOfClients; ++i) {
         ...
         if (FD_ISSET(fd, &fdread)) {
            ...
            if (strcmp(clients[i].client_id, "") ==
                0)  // Client hasn't had a name
            {
               if (strncmp(buff, "client_id:", 10) == 0) {
                  ...
                  // Wrong format
                  send(fd, "Please enter your name (client_id:client_name)", 46,
                       0);
               }
            } else  // Client has already had a name
            {
               sprintf(sendBuff, "%04d/%02d/%02d %02d:%02d:%02d %s:%s",
                       ...
                       tm.tm_min, tm.tm_sec, clients[i].client_id, buff);
               int length = strlen(clients[i].client_id) + ret + 21;
               for (int j = 0; j < numberOfClients; ++j) {
                  // Boardcast message to other clients
                  if (i != j) send(clients[j].fd, sendBuff, length, 0);
               }
            }
         }
      }
